...
class WalletService {
  ...
  static async createMetamaskWallet(userId, publicKey, locale) {
    let user = await UserModel.findOne({ id: userId });
    if (!user) {
      return { response: false, message: "You are not registered user.", data: null };
    }
    let wallet = new Wallet("polygon-mainnet").createMetamaskWallet(userId, publicKey)
    ...
    return { response: true, message: "success", data: { publicKey: wallet.publickey } }
  }
  ...
  static async createWallet(userId, keyphrase, locale) {
    ...
    console.log("keyphrase asdf:", keyphrase)
