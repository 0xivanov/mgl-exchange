...
const Launch = () => {
  ...
  const connect = async () => {
    try {
      ...
      await completeMetamaskCreation(accounts?.[0])
    } catch (err) {
  ...
  async function completeMetamaskCreation(publicKey) {
    setAuthToken(localStorage.jwtToken)
    const response = await axios.post(serverUrl + "wallets/metamaskCreate", {
      publicKey: publicKey,
                ...
                <Row className="walletSelect div-wrap">
                  <Col className="div-relative">
                    <Row className="wallet-detail">
                      <Col span={24}>
                        <img src="./assets/img/metamask.png" style={{ width: 135, height: 135 }} />
                      </Col>
                    </Row>
                    ...
                    <Row className="wallet-detail header">
                      {t("Access your existing metamask wallet here")}
                    </Row>
                    <Row className="wallet-detail wallet-button">
                      <Col span={24}>
                        <Button className="access-button" onClick={connect}>{t("CONNECT WITH METAMASK")}</Button>
                      </Col>
                    </Row>
                  </Col>
                </Row>
                  ...
                  <Col className="div-relative">
                    ...
                    <Row className="wallet-detail wallet-button">
                      <Col span={24}>
                        {
                          verify ?
                            <Button className="access-button  mt-2" onClick={completeCreation}>{t("COMPLETE CREATION")}</Button>
                            :
