...
export async function getTokenBaseInfo(address, networkUrl) {
  ...
  if (address === "0x0000000000000000000000000000000000001010" && (networkUrl === "bsc-mainnet" || networkUrl === "bsc-testnet")) {
    return {
      name: "BNB",
      symbol: "BNB",
      decimal: 18
...
export async function getTokenBalance(tokenAddress, decimal, networkUrl, publicKey) {
  console.log("get balance", tokenAddress, decimal, networkUrl, publicKey)
  try {
    let rpcUrl = getRpcUrl(networkUrl);
    if (!rpcUrl)
      return;

    if (tokenAddress === "0x0000000000000000000000000000000000001010" && (networkUrl === "bsc-mainnet" || networkUrl === "bsc-testnet")) {
      let web3 = new Web3(rpcUrl);
...
export async function getTokenPriceInUsd(network, tokenAddr) {
  try {
    ...
    if (network.url === "bsc-testnet") console.log("routerContract", routerContract)
